...
**Real-world failure:**
User runs `kuro run development -t "Add caching for step outputs"`. The architect interprets "caching" as in-memory only. The implementer builds in-memory caching correctly. The reviewer confirms implementation matches the design. All agents succeed, but the user wanted disk-based caching.
